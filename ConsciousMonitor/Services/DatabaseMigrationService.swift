...
class DatabaseMigrationService: ObservableObject {
    ...
    private func loadSourceData() async throws -> SourceData {
        return try await withCheckedThrowingContinuation { continuation in
            let eventStorageService = self.eventStorageService
            let dataStorage = self.dataStorage
            
            DispatchQueue.global(qos: .utility).async {
                do {
                    // Load from EventStorageService (modern approach)
                    let events = eventStorageService.events
                    ...
                    // Load legacy data
                    legacyEvents = dataStorage.loadEvents()
                    
                    // Load context switches synchronously using semaphore
                    let semaphore = DispatchSemaphore(value: 0)
                    var contextSwitchResult: [ContextSwitchMetrics] = []
                    var contextSwitchError: Error?
                    
                    dataStorage.loadContextSwitches { result in
                        switch result {
                        case .success(let switches):
                            contextSwitchResult = switches
                        case .failure(let error):
                            contextSwitchError = error
                        }
                        semaphore.signal()
                    }
                    ...
                    if let error = contextSwitchError {
                        print("Warning: Failed to load context switches: \(error)")
                        legacyContextSwitches = []
                    } else {
                        legacyContextSwitches = contextSwitchResult
                    }
    ...
    // MARK: - Synchronous Data Loading (removed due to concurrency issues)
