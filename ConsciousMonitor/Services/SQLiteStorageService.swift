...
class SQLiteStorageService: ObservableObject, StorageServiceProtocol {
    nonisolated(unsafe) static let shared = SQLiteStorageService()
    ...
    private func prepareStatements() throws {
        ...
        // Prepare event insert statement (simplified approach)
        let eventInsertSQL = """
            INSERT OR REPLACE INTO app_activation_events 
            (id, timestamp, app_name, bundle_identifier, chrome_tab_title, chrome_tab_url, 
             site_domain, category_name, session_id, session_start_time, session_end_time, 
             is_session_start, is_session_end, session_switch_count, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        insertEventStatement = try db.prepare(eventInsertSQL)
        
        // Prepare context switch insert statement (simplified approach)
        let switchInsertSQL = """
            INSERT OR REPLACE INTO context_switch_metrics 
            (id, from_app, to_app, from_bundle_id, to_bundle_id, timestamp, 
             time_spent, switch_type, from_category, to_category, session_id, created_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        insertSwitchStatement = try db.prepare(switchInsertSQL)
    }
}
                    ...
                    DispatchQueue.main.async {
                        ...
                        for event in sortedEvents {
                            if let existingIndex = self.events.firstIndex(where: { $0.id == event.id }) {
                                ...
                                // Find proper insertion point to maintain chronological order
        ...
        return try await withCheckedThrowingContinuation { continuation in
            dbQueue.async {
                do {
                    ...
                    // Update in-memory cache if this is a full load
                    if startDate == nil && endDate == nil && limit == nil {
                        DispatchQueue.main.async {
                            self.events = events
                        }
                    }
...
extension SQLiteStorageService {
        ...
        try await withCheckedThrowingContinuation { continuation in
            dbQueue.async {
                do {
                    ...
                    // Update in-memory cache
                    DispatchQueue.main.async {
                        if let existingIndex = self.contextSwitches.firstIndex(where: { $0.id == contextSwitch.id }) {
                            self.contextSwitches[existingIndex] = contextSwitch
                        } else {
    ...
    private func contextSwitchFromRow(_ row: Row) throws -> ContextSwitchMetrics? {
        ...
        // Create context switch using the main initializer to ensure proper switch type calculation
                ...
                do {
                    ...
                    for row in try statement.run([startDate, endDate]) {
                        ...
                        let stat = AppUsageStat(
                            ...
                            category: category,
                            siteBreakdown: nil // TODO: Implement site breakdown when needed
                        )
...
extension SQLiteStorageService {
    ...
    /// Migrate data from JSON-based storage services
    func migrateFromJSONStorage() async throws {
        await MainActor.run {
            isLoading = true
        ...
        do {
            // Load existing data from EventStorageService and DataStorage
            let eventStorageService = EventStorageService.shared
            ...
            await withCheckedContinuation { continuation in
                if eventStorageService.isInitialLoadComplete {
                    ...
                    observer = NotificationCenter.default.addObserver(
                        forName: .sqliteEventsLoadCompleted,
                        object: nil,
                        queue: .main
                    ) { [weak observer] _ in
            ...
            let existingEvents = eventStorageService.events
            
            // Load context switches from DataStorage
            let existingContextSwitches = try await dataStorage.loadContextSwitches()
            
            let totalItems = existingEvents.count + existingContextSwitches.count
            var processedItems = 0
            
            print("Starting migration: \(existingEvents.count) events, \(existingContextSwitches.count) context switches")
            ...
            let switchBatches = existingContextSwitches.chunked(into: 100)
...
extension AppCategory {
    static func fromName(_ name: String) -> AppCategory {
        // Use CategoryManager to get the category, falling back to the name-based lookup
        if let category = CategoryManager.shared.allAvailableCategories.first(where: { $0.name.lowercased() == name.lowercased() }) {
            return category
        }
...
extension Notification.Name {
    static let sqliteEventsLoadCompleted = Notification.Name("SQLiteEventsLoadCompleted")
}